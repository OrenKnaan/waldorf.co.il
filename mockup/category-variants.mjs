...
const COLLAGE_CSS = `
  ...
  .collage-grid .tile{overflow:hidden;box-shadow:0 8px 20px rgba(30,20,12,.28)}
...
const TEXTURE_CSS = `
  /* ===== variant: מרקם צבעי מים על פני כל הדף (זמני). Two fixed,
     ...
     .page-tex holds watercolor-clouds.webp (its own colour is blue) with
     background-blend-mode:multiply against .page-wash showing through, and
     filter:hue-rotate to match the category: filter has to sit on its own
     element rather than on body, or it would just as happily rotate
     .page-wash's already-correct colour a second time. multiply keeps the
     photograph's white paper white (multiplying white changes nothing) and
     only its blue blooms tint the ground under them. */
  ...
  .page-tex{position:fixed;inset:0;z-index:-1;pointer-events:none;
    ...
    background-blend-mode:multiply;filter:hue-rotate(var(--tex-hue))}
`;
